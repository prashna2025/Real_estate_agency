      ...
      {loading ? <p className="font-serif italic text-charcoal-muted">Loading inquiries...</p> : visibleInquiries.length === 0 ? (
        ...
        <div className="bg-white border border-stone rounded-sm overflow-x-auto">
          <table className="w-full min-w-[760px] text-left">
            <thead className="border-b border-stone bg-cream-dark/40"><tr><th className="p-4 text-xs uppercase tracking-wider text-charcoal-muted">Client</th><th className="p-4 text-xs uppercase tracking-wider text-charcoal-muted">Property</th><th className="p-4 text-xs uppercase tracking-wider text-charcoal-muted">Message</th><th className="p-4 text-xs uppercase tracking-wider text-charcoal-muted">Status</th></tr></thead>
            <tbody>{visibleInquiries.map((inquiry) => <tr key={inquiry._id} className="border-b border-stone last:border-0 align-top"><td className="p-4"><p className="font-medium">{inquiry.name}</p><a href={`mailto:${inquiry.email}`} className="text-sm text-terracotta hover:underline">{inquiry.email}</a><p className="text-sm text-charcoal-muted">{inquiry.phone}</p></td><td className="p-4 text-sm">{inquiry.propertyId ? <Link to={`/property/${inquiry.propertyId.slug}`} target="_blank" className="text-terracotta hover:underline">{inquiry.propertyId.title}</Link> : 'Property removed'}</td><td className="p-4 max-w-sm text-sm text-charcoal-muted">{inquiry.message}</td><td className="p-4"><select value={inquiry.status} onChange={(event) => updateStatus(inquiry._id, event.target.value)} className="border border-stone bg-cream px-2 py-2 text-sm rounded-sm">{statuses.map((status) => <option key={status}>{status}</option>)}</select></td></tr>)}</tbody>
          </table>
        </div>
      )}
    </div>
